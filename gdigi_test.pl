...
sub set_and_test ($$$) {
    ...
    if (gdigi_get_parameter($id, $pos, $new_val) < 0) {
...
sub test_tree ($) {
    ...
    if ($$node{'type'} eq "Effect Page") {
        ...
        iprint "Testing Effect '$name' id $id position $pos\n";

        if (not ($id == 0 and $op == 0) and
            gdigi_get_parameter($id, $pos, $effect_val) < 0) {
        ...
        if (gdigi_get_parameter($id, $pos, $orig_val) < 0) {
        ...
        if (gdigi_get_parameter($id, $pos, $orig_val) < 0) {
            die("Failed to get parameter for setting $name id $id position $pos\n");
        }

        iprint "Testing setting $name id $id position $pos orig value $orig_val\n";
